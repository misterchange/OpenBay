...
**Terminal 2 — a seeder (hosts the whole model):**
```bash
...
```
Tokens stream back from the seeder via the coordinator.
...
Start a second seeder on another port — or another machine on your LAN:

```bash
# same machine, second instance
python -m openbay.seeder --model llama3.2 --port 9001
